...
void handle_page_fault(regs_context_t *regs, uint64_t stval, uint64_t scause){
    ...
    // 页面存在
    if(pte && get_attribute(*pte, _PAGE_PRESENT)){
        // cow时分配的只读页面
        if(scause==EXCC_STORE_PAGE_FAULT && get_attribute(*pte, _PAGE_WRITE)==0){
            uint64_t kva_old = get_kva_of(stval, current_running[cpu_id]->pgdir);
            // 先取消原本映射
            map_page_helper(stval, 0, current_running[cpu_id]->pgdir);
            uint64_t kva_new = alloc_page_helper(stval, current_running[cpu_id]->pgdir);
            // 复制原本页面内容
            memcpy(kva_new, kva_old, NORMAL_PAGE_SIZE);
        }
    }
    // 页面不存在，建立映射
    else{
